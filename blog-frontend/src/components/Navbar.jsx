    ...
    <header className="bg-white shadow">
      <div className="container mx-auto flex justify-between items-center p-4">
        <div className="flex items-center space-x-4">
          ...
          <Link to="/" className="px-3 py-1 rounded hover:bg-gray-100">All Posts</Link>
          {token && <Link to="/new" className="px-3 py-1 bg-green-500 text-white rounded">New Post</Link>}
          {token && <Link to="/change-password" className="px-3 py-1 bg-yellow-500 text-white rounded">Change Password</Link>}
        </div>
        <div>
          {token ? (
